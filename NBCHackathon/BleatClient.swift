...
class BleatClient: NSObject {


    let pushEndpoint: String = UAirship.shared().config.deviceAPIURL + "/api/push/"

    func bleatAlert(_ alert: String, soundFile:String?) {

        bleatAlertWithPayload(generatePayload(alert, soundFile: soundFile)!)
    }

    func bleatAlertWithPayload(_ payload: [String : Any]) {
